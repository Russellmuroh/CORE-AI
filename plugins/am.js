...
const AntiDelete = async (m, sock) => {
  if (!sock?.ev?.on) {
    console.error('AntiDelete plugin failed: sock.ev.on is undefined');
    return;
  }

  // Anti-Delete feature — only setup once
  if (!sock._antiDeleteSetup) {
    sock.ev.on('messages.update', async (updates) => {
      for (let update of updates) {
        if (update.key?.fromMe) return;
        if (update.update !== 'delete') return;

        const jid = update.key.remoteJid;
        const msgId = update.key.id;

        if (!config.AUTO_ANTI_DELETE) return;
        ...
        try {
          ...
          console.error('❌ Failed to retrieve deleted message:', err);
        }
      }
    });

    sock._antiDeleteSetup = true; // Prevent multiple listeners
  }

  // Command toggle: antidelete on/off
  const msg = m.message?.conversation || '';
  if (!msg || m.key.fromMe) return;

  const text = msg.trim().toLowerCase();
  if (text === 'antidelete on') {
    config.AUTO_ANTI_DELETE = true;
    await sock.sendMessage(m.key.remoteJid, { text: 'Anti-Delete is now *enabled*.' });
  } else if (text === 'antidelete off') {
    config.AUTO_ANTI_DELETE = false;
    await sock.sendMessage(m.key.remoteJid, { text: 'Anti-Delete is now *disabled*.' });
  }
};
