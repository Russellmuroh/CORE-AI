...
// Function to get real-time formatted date (Africa/Nairobi Time Zone)
const getRealDate = () => {
    const now = new Date();
    return now.toLocaleDateString('en-GB', { timeZone: 'Africa/Nairobi', year: 'numeric', month: 'long', day: 'numeric' });
};

// Function to get real-time formatted time (Africa/Nairobi Time Zone)
const getRealTime = () => {
    const now = new Date();
    return now.toLocaleTimeString('en-GB', { timeZone: 'Africa/Nairobi', hour: '2-digit', minute: '2-digit', hour12: true });
};
