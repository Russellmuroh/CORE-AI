...
// Function to fetch user stats (you can replace this with actual logic)
const getUserStats = async (user) => {
    // Dummy stats, replace with actual database or logic if needed
    return {
        menuCount: 5, // Example: how many times the user has used the bot today
    };
};
...
const menu = async (m, Matrix) => {
    ...
    const lastUpdated = moment().format('LLLL'); // Get last updated timestamp
    const userStats = await getUserStats(m.sender); // Fetch user stats
...
⏳ It seems you're not responding. Would you like to see the menu again?

Reply with *Menu* to start.
`;

    const menuImageUrl = 'https://files.catbox.moe/7jt69h.jpg';  // Direct URL to the image

    // Send the main menu image with caption only once
    ...
    // Timeout handling to prompt the user if no response after a certain time (e.g., 30 seconds)
    setTimeout(() => {
    ...
    // Respond only once to the user's selection (1-10) and not continuously
    if (cmd === 'menu') {
        Matrix.sendMessage(m.from, {
            text: mainMenu,
            contextInfo: { mentionedJid: [m.sender] }
    ...
    // Define responses for each menu option
    const menus = {
        "1": `
🔽 *DOWNLOAD MENU* 🔽
...
`
    ...
    // If a valid menu option is selected, respond with that specific menu
    if (menus[cmd]) {
        Matrix.sendMessage(m.from, {
            text: menus[cmd],
            contextInfo: { mentionedJid: [m.sender] }
