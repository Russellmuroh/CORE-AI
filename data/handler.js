...
const Handler = async (chatUpdate, sock, logger) => {
  try {
    ...
    // **Integrate GroupManager** for group-related commands
    const groupManager = new GroupManager();
    
    if (m.isGroup) {
      // Handle commands without a prefix, based on message content
      const messageContent = m.body.trim().toLowerCase();

      if (messageContent === 'add' || messageContent === 'kick' || messageContent === 'promote' || messageContent === 'demote' || messageContent === 'open' || messageContent === 'close' || messageContent === 'setdesc' || messageContent === 'setname' || messageContent === 'antilink' || messageContent === 'antidelete') {
        await groupManager.handleCommand(sock, m, { isAdmin: isAdmins, isBotAdmin: isBotAdmins, prefix: '' });
      }
    }
