...
class GroupManager {
  constructor() {
    this.groupSettings = new Map(); // Stores { antiLink: boolean, welcomeMsg: string }
  }

  async handleCommand(sock, m, { isAdmin, isBotAdmin, prefix }) {
    if (!m.isGroup || !isAdmin) return;

    const [cmd, ...args] = m.body.slice(prefix.length).trim().split(' ');
    ...
    try {
      switch(cmd.toLowerCase()) {
        case 'add':
          ...
          await this.addMember(sock, m.from, content);
          break;

        case 'antilink':
          await this.toggleAntiLink(m.from);
          await sock.sendMessage(m.from, { 
            text: `🔗 Anti-link ${this.groupSettings.get(m.from)?.antiLink ? 'ON' : 'OFF'}` 
          });
          break;

        case 'setwelcome':
          await this.setWelcomeMsg(m.from, content);
          await sock.sendMessage(m.from, { 
      ...
      await sock.sendMessage(m.from, { text: `❌ ${error.message}` });
    }
  }
  ...
  async handleMessages(sock, m) {
    ...
    if (this.groupSettings.get(m.from)?.antiLink) {
      const urlRegex = /https?:\/\/[^\s]+/g;
      if (urlRegex.test(m.body)) {
        await sock.sendMessage(m.from, { delete: m.key });
        await sock.sendMessage(m.from, {
          text: `⚠️ @${m.sender.split('@')[0]} No links allowed!`,
          mentions: [m.sender]
        });
      }
    }
    ...
    // Welcome new members
    if (m.message?.protocolMessage?.type === 2) { // GROUP_PARTICIPANT_ADD
      const welcomeMsg = this.groupSettings.get(m.from)?.welcomeMsg;
      if (welcomeMsg) {
  ...
  // Helper methods
  async addMember(sock, groupJid, phone) {
...
const Handler = async (chatUpdate, sock, logger) => {
  try {
    ...
    // 2. Group protections (anti-link, welcome messages)
    if (m.isGroup) {
      await groupManager.handleMessages(sock, m);
    }

    // 3. Existing antilink handler (keep your original)
    // await handleAntilink(m, sock, logger, isBotAdmin, isAdmin, isCreator);

    // 4. Sheng AI processing
    await shengCommand(m);
    await shengChat(m);

    // 5. Plugin loader (unchanged)
    const pluginDir = path.resolve(__dirname, '..', 'plugins');
    try {
      const pluginFiles = await fs.readdir(pluginDir);
      for (const file of pluginFiles) {
        if (file.endsWith('.js') && !file.includes('antilink')) { // Skip antilink.js if exists
